...
class Propagator2D(torch.nn.Module):
    ...
    def __init__(self, nz, nx, dz, dt, 
        ...
        if order == 4:
            ...
            self.kernel2D = self.___make_tensor([[0, 0, -(dt / dz)**2 / 12, 0, 0],
                                                ...
                                                [0, 0, -(dt / dz)**2 / 12, 0, 0]])
        if order == 3:
            self.kernelz1D = self.___make_tensor([[0,        ],
                                                  [(1/24)/dz ],
                                                  [(-9/8)/dz ],
                                                  [(9/8)/dz  ],
                                                  [(-1/24)/dz]])
            
            self.kernelx1D = self.___make_tensor([[0, (1/24)/dx,(-9/8)/dx, (9/8)/dx, (-1/24)/dx]])

            
            self.kernelz1D_U = self.___make_tensor([[(1/24)/dz ],
                                                    [(-9/8)/dz ],
                                                    [(9/8)/dz  ],
                                                    [(-1/24)/dz],
                                                    [0         ]])
            ...
            self.kernelx1D_U = self.___make_tensor([[(1/24)/dx,(-9/8)/dx, (9/8)/dx, (-1/24)/dx,0]])

            # print("forward_self.kernelz1Dshape", self.kernelz1D.shape)
            self.padding_num = 2
        ...
        if order == 6:
            ...
            self.kernelx1D = self.___make_tensor([[0, (-3/640)/dz, (25/384)/dz, (-75/64)/dz, (75/64)/dz, (-25/384)/dz, (3/640)/dz  ]])

            
            self.kernelz1D_U =  self.___make_tensor([[(-3/640)/dz ],
                                                     [(25/384)/dz ],
                                                     [(-75/64)/dz ],
                                                     [(75/64)/dz  ],
                                                     [(-25/384)/dz],
            ...
            self.kernelx1D_U = self.___make_tensor([[(-3/640)/dz, (25/384)/dz, (-75/64)/dz, (75/64)/dz, (-25/384)/dz, (3/640)/dz, 0  ]])
        ...
        if order == 8:
            ...
            self.kernelx1D = self.___make_tensor([[0, (5/7168)/dz, -(49/5120)/dz, (245/3072)/dz, (-1225/1024)/dz, (1125/1024)/dz, (-245/3072)/dz, (49/5120)/dz, (-5/7168)/dz  ]])

            
            self.kernelz1D_U =  self.___make_tensor([[(5/7168)    /dz  ],
                                                     [-(49/5120)  /dz  ],
                                                     [(245/3072)  /dz  ],
                                                     [(-1225/1024)/dz  ],
                                                     [(1125/1024) /dz  ],
            ...
            self.kernelx1D_U = self.___make_tensor([[ (5/7168)/dz, -(49/5120)/dz, (245/3072)/dz, (-1225/1024)/dz, (1125/1024)/dz, (-245/3072)/dz, (49/5120)/dz, (-5/7168)/dz, 0  ]])
        ...
        if order == 10:
            ...
            self.kernelx1D = self.___make_tensor([[0,(-35/294912)/dz, (405/229376)/dz, (-567/40960)/dz, (735/8192)/dz, (-19845/16384)/dz, (19845/16384)/dz, (-735/8192)/dz, (567/40960)/dz, (-405/229376)/dz, (35/294912) /dz]])

            
            self.kernelz1D_U =  self.___make_tensor([[  (-35/294912)/dz],
                                                     [  (405/229376)/dz],
                                                     [  (-567/40960)/dz],
                                                     [  (735/8192)  /dz],
                                                     [(-19845/16384)/dz],
            ...
            self.kernelx1D_U = self.___make_tensor([[(-35/294912)/dz, (405/229376)/dz, (-567/40960)/dz, (735/8192)/dz, (-19845/16384)/dz, (19845/16384)/dz, (-735/8192)/dz, (567/40960)/dz, (-405/229376)/dz, (35/294912) /dz, 0]])
    ...
    #def ___make_tensor_1D_kernel(self, a):
    ...
    def ___step_rnncell(self, source, vx, vz, vx_x, vx_z, vz_x, vz_z,txx, tzz, txz, txx_x, txx_z, tzz_x, tzz_z, txz_x, txz_z):
        ...
        for im in range(vx.shape[1]): # adding source for each shot record im represent the shot index  
            if self.freeSurface:
                ...
                vz[:, im, source[1][0][im], source[1][1][im]+self.npad] += source[0]
            else:
                #txx[:, im, source[1][0][im]+self.npad, source[1][1][im]+self.npad] += source[0]  
                vz[:, im, source[1][0][im]+self.npad, source[1][1][im]+self.npad] += source[0]
        '''
        '''
        for im in range(vx.shape[1]): # adding source for each shot record im represent the shot index  
            if self.freeSurface:
                #vx[:, im, source[1][0][im], source[1][1][im]+self.npad] += source[0]
                tzz[:, im, source[1][0][im], source[1][1][im]+self.npad] += source[0]
            else:
                #vx[:, im, source[1][0][im]+self.npad, source[1][1][im]+self.npad] += source[0]
                tzz[:, im, source[1][0][im]+self.npad, source[1][1][im]+self.npad] += source[0]
        '''

        var_txx_x = F.conv2d(txx, self.kernelx1D.repeat(txx.shape[1], 1, 1, 1), \
        padding=(0 ,self.padding_num), groups=txx.shape[1])/self.rho_tensor
        
        var_txz_z = F.conv2d(txz, self.kernelz1D_U.repeat(txz.shape[1], 1, 1, 1), \
        padding=(self.padding_num, 0), groups=txx.shape[1])/self.rho_tensor
        
        var_txz_x = F.conv2d(txz, self.kernelx1D_U.repeat(txz.shape[1], 1, 1, 1), \
        padding=(0, self.padding_num), groups=txx.shape[1])/self.rho_tensor
        
        var_tzz_z = F.conv2d(tzz, self.kernelz1D.repeat(tzz.shape[1], 1, 1, 1), \
        padding=(self.padding_num, 0), groups=txx.shape[1])/self.rho_tensor
        ...
        var_vx_x = F.conv2d(vx, self.kernelx1D_U.repeat(vx.shape[1], 1, 1, 1), \
        padding=(0, self.padding_num), groups=txx.shape[1])
        
        var_vx_z = F.conv2d(vx, self.kernelz1D.repeat(vx.shape[1], 1, 1, 1), \
        padding=(self.padding_num, 0), groups=txx.shape[1])
        
        var_vz_x = F.conv2d(vz, self.kernelx1D.repeat(vz.shape[1], 1, 1, 1), \
        padding=(0, self.padding_num), groups=txx.shape[1])
        
        var_vz_z = F.conv2d(vz, self.kernelz1D_U.repeat(vx.shape[1], 1, 1, 1), \
        padding=(self.padding_num, 0), groups=txx.shape[1])
    ...
    segment_sources,vx,vz,vx_x,vx_z,vz_x,vz_z,txx,tzz,txz,txx_x,txx_z,tzz_x,tzz_z,txz_x,txz_z):
        ...
        if self.freeSurface:
                ...
                yt_pred_vz_z  = vz_z[0, :, [zr for zr in segment_sources[2][0]], [xr+self.npad-1 for xr in segment_sources[2][1]] ]
        else:
        ...
        #regularizer  = None
        return vx, vz, vx_x, vx_z, vz_x, vz_z, txx, tzz, txz,txx_x, txx_z, tzz_x, tzz_z, txz_x, txz_z,\
        yt_pred_x, yt_pred_z, yt_pred_vz_z, regularizer, self.vp_tensor,self.vs_tensor,self.rho_tensor, ax, az
...
class rnn2D(torch.nn.Module):
    ...
    def forward(self, vp_tensor,vs_tensor,rho_tensor, segment_wavelet, option=0):
        ...
        for it in range(segment_wavelet.shape[1]):
            ...
            #segment_sources[4])
            vx, vz, vx_x, vx_z, vz_x, vz_z, txx, tzz, txz,txx_x, txx_z, tzz_x, tzz_z, txz_x, txz_z,\
            seg_ytPred_x, seg_ytPred_z, seg_ytPred_vz_z, regularizer, self.vp_tensor,self.vs_tensor,self.rho_tensor, \
            ax,az = \
            self.fd(vp_tensor,vs_tensor,rho_tensor, \
                    segment_sources, vx,vz,vx_x, vx_z, vz_x, vz_z,txx, tzz, txz, txx_x, txx_z, tzz_x, tzz_z, txz_x, txz_z)
            segment_ytPred_x.append(seg_ytPred_x)
            segment_ytPred_z.append(seg_ytPred_z)
            segment_ytPred_vz_z.append(seg_ytPred_vz_z)
        ...
        segment_ytPred_vz_z = torch.stack(segment_ytPred_vz_z, dim=-2)  # shape: [num_vels, num_shots, nt(len_tSeg), num_receivers]

...
if length(varargin) == 1
    itmax = varargin{1}.MaxIterations;
    nsamplei = varargin{1}.InitialSample;
    nsample = varargin{1}.ReSamples;
    ncells = varargin{1}.CellsResampled;
    mfbreak = varargin{1}.MisfitBreak;
    ncbreak = varargin{1}.NConstBreak;
    ...
    while iarg <= (length(varargin))
        switch lower(varargin{iarg})
            case 'maxiterations'
                itmax = varargin{iarg+1};
                iarg = iarg + 2;

            case 'initialsample'
                nsamplei = varargin{iarg+1};
                iarg = iarg + 2;
            
            case 'resamples'
                nsample = varargin{iarg+1};
                iarg = iarg + 2;
            
            case 'cellsresampled'
                ncells = varargin{iarg+1};
                iarg = iarg + 2;
            
            case 'misfitbreak'
                mfbreak = varargin{iarg+1};
                iarg = iarg + 2;
        
            case 'nconstbreak'
                ncbreak = varargin{iarg+1};
                iarg = iarg + 2;
